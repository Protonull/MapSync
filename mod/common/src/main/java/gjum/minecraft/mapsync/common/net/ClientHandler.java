	...
	public void channelRead(ChannelHandlerContext ctx, Object packet) {
		try {
			if (!client.isEncrypted()) {
				if (packet instanceof S2C_EncryptionRequestPacket pktEncryptionRequest) {
					client.setUpEncryption(ctx, pktEncryptionRequest);
				} else throw new Error("Expected encryption request, got " + packet);
			} else if (packet instanceof BID_ChunkDataPacket pktChunkTile) {
				getMod().handleSharedChunk(pktChunkTile.chunkTile());
			} else if (packet instanceof S2C_RegionTimestampsPacket pktRegionTimestamps) {
				getMod().handleRegionTimestamps(pktRegionTimestamps, client);
			} else if (packet instanceof S2C_RegionCatchupResponsePacket pktCatchup) {
				for (CatchupChunk chunk : pktCatchup.chunks()) {
					chunk.syncClient = this.client;
				}
				getMod().handleCatchupData((S2C_RegionCatchupResponsePacket) packet);
