...
public class CatchupLogic {
	...
	public void addCatchupChunks(List<CatchupChunk> catchupChunks) {
		...
		if (!dimensionState.dimension.equals(catchupDim)) {
			MapSyncMod.LOGGER.warn("Catchup chunks from wrong dimension " + catchupDim + ", expected " + dimensionState.dimension);
			return;
		}
		synchronized (this.catchupChunks) {
			for (CatchupChunk chunk : catchupChunks) {
				...
				MapSyncMod.debugLog("COMPARING " + (current_timestamp < chunk.timestamp()) + " for " + chunk.chunk_x() + " " + chunk.chunk_z());
				if (current_timestamp < chunk.timestamp()) {
					this.catchupChunks.add(chunk);
				}
			}
			MapSyncMod.debugLog("now have " + this.catchupChunks.size() + " catchup chunks");
		}
	...
	synchronized void maybeRequestMoreCatchup() {
		int WATERMARK_REQUEST_MORE = MapSyncMod.CONFIG.getCatchupWatermark();
		...
		if (queueSize < WATERMARK_REQUEST_MORE) {
			...
			MapSyncMod.getMod().requestCatchupData(chunksToRequest);
		}
	}
